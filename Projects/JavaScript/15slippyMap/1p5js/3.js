...
let cachedKeys = new Array(MAX_CACHED_TILES).fill(-1);
let cacheIndex = 0;
...
function trimCache() {
    cacheIndex += 1;
    if (cacheIndex == MAX_CACHED_TILES) {
        cacheIndex = 0;
    }

    cachedKeys[cacheIndex] = -1; // Mark this slot as empty
}
...
function getTile(z, y, x) {
    ...
    // Tile is in cache
    const index = cachedKeys.indexOf(tileID);
    if (index !== -1) {
        const arrayIndex = index * TILE_SIZE * TILE_SIZE * 4;
    ...
    if (!isLoading) {
        ...
        setTimeout(() => {
            loadImage(`map/${z}/${y}/${x}.png`, (img) => {
                ...
                const arrayIndex = cacheIndex * TILE_PIXEL_COUNT;

                tileCache.set(img.pixels, arrayIndex);

                cachedKeys[cacheIndex] = tileID;
