...
subroutine newton_raphson(X0,SOL,tol,max_iter,f1)
    REAL*8,intent(inout)    ::  X0(:), SOL(:), tol
...
subroutine all_roots2D(sol,numraices,tol1,f1)
    REAL*8,intent(inout)    ::  sol(:,:), tol1
    integer,intent(in)      ::  numraices

    integer     ::  i, cont, max_interval, max_iter, j, k, m
    REAL*8      ::  x0(2)
    ...
    do i = 1,max_interval
        max_iter = 707777
        x0(1) = (-0.1+0.1*i)*cos(0.1*i)
        x0(2) = (-0.1+0.1*i)*sin(0.1*i)
        cont = cont+1
        call newton_raphson(x0,sol(cont,:),tol1,max_iter,f1)
        do j = 1,cont-1
            k = 0
            do m = 1,2
                if(abs(sol(j,m)-sol(cont,m))<10.d0**(-6)) k = k+1
            enddo
            write(*,*) k
            if(k==2) EXIT
        enddo
        if(max_iter==707777.OR.k==2) cont = cont - 1
        if(max_iter/=707777.AND.k/=2) write(*,*) sol(cont,:), "lmao"
        if(cont==numraices) EXIT
    enddo
        write(*,*) i
