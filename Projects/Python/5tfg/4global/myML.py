...
class kriging:
    ...
    def calculateweights(self):
        ...
        self.MLm = np.sum(Rinv @ self.ty.T)/np.sum(Rinv)
        self.weights = (self.ty-self.MLm) @ Rinv
        return
    
    def interpol(self, points):
        m = points.shape[1]
        phinm = np.zeros((self.n,m))
        for i in range(self.n):
            for j in range(m):
                phinm[i,j] = basisfunc(points[0,j]-self.tx[0,i],self.p,self.theta)
        y = self.MLm + self.weights @ phinm
        return y
...
"""
#############################################################
#Simulated Annealing (seed=np.array([[1],[2]]), lims=[[0,1],[-2,300]], sigma=1, @func to maximize, iters=4000)
