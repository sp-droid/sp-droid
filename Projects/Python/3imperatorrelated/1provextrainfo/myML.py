...
## Loss functions
def logloss(y_train, y_pred):                           #Cross-entropy, log loss --> Classification problems
    m = y_train.shape[1]

    #return -1/m*(y_train*np.log(y_pred) + (1-y_train)*np.log(1-y_pred))
    return -1/m*(xlogy(y_train, y_pred) + xlog1py(1-y_train, -y_pred))
def d_logloss(y_train, y_pred):
    m = y_train.shape[1]
    return 1/m*(y_pred - y_train)/(y_pred - y_pred**2+1e-20)

def MSE(y_train, y_pred):                                           #Mean squared error  --> Regression without outliers
    m = y_train.shape[1]
    return 2/m*(y_train-y_pred)**2
def d_MSE(y_train, y_pred):
    m = y_train.shape[1]
    return 1/m*(y_pred-y_train)
...
class ANN:
    ...
    def __init__(self, nodes, activation, lossname):
        ...
        self.Xnorm, self.Ynorm = DEFAULT, DEFAULT
        # Layer creation
    ...
    def defineNorm(self, normType, dataset=DEFAULT, minimum=DEFAULT, maximum=DEFAULT):
        if normType=='Input':
    ...
    ## Model feedforward
    def run(self, x):
        if self.Xnorm is not DEFAULT: x = self.Xnorm.normalize(x)
        x = self.typeChecks(x)
        ...
        y = x.T
        if self.Ynorm is not DEFAULT: y = self.Ynorm.recover(y)
        return y
    ...
    def trainGA(self, x_train, y_train, generations, popsize, iniOffspring=2, maxminMutation=[0.8,0.02], tqdmDisable=False):
        if self.Xnorm is not DEFAULT: x_train = self.Xnorm.normalize(x_train)
        if self.Ynorm is not DEFAULT: y_train = self.Ynorm.normalize(y_train)
    ...
    def train(self, x_train: np.ndarray, y_train: np.ndarray, epochs, optimizer, lr, lambd, batchSize: int=0, trainTestSplit=0.9, tqdmDisable=False):
        ...
        for epoch in pbar:
            ...
            Jtrain = np.sum(self.loss(y_train[...,idxTrain], y))
            if trainTestSplit!=1: 
                y = self.trainRun(x_train[...,idxTest])
                Jtest = np.sum(self.loss(y_train[...,idxTest], y))
            pbar.set_postfix_str(f'loss: {Jtrain:.5f}, testLoss: {Jtest:.5f}')
    ...
    def storelayers(self):
        with open('myML/NNdata.pkl', 'wb') as file: #wb is necessary, write-binary
            pickle.dump([self.layers, self.loss, self.d_loss], file)
        print('Stored in myML/NNdata.pkl')

    def loadlayers(self, DIR):
        with open(DIR, 'rb') as file: #rb is necessary, read-binary
            self.layers, self.loss, self.d_loss = pickle.load(file)
...
class norm0to1: #Knowing the initial dataset
    ...
    '''
    def __init__(self, dataset=DEFAULT, minimum=DEFAULT, maximum=DEFAULT):
        if dataset is DEFAULT and minimum is DEFAULT and maximum is DEFAULT: raise ValueError('Introduce either the dataset or the limits for each dimension')
        elif minimum is not DEFAULT and maximum is not DEFAULT:
            self.d = len(maximum)
